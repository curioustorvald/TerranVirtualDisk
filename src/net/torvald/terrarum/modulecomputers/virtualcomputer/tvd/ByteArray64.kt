...
//            byteArray.forEachIndexed { i, byte -> ba64[i.toLong()] = byte }
    ...
    fun appendBytes(bytes: ByteArray) {
        checkMutability()
        ensureCapacity(size + bytes.size)
        val chunkOffset = (size % BANK_SIZE).toInt()
        var currentBankNumber = (size / BANK_SIZE).toInt()
        val remainingInHeadBank = BANK_SIZE - chunkOffset // how much space left in the current (= head) bank
        ...
        System.arraycopy(bytes, srcCursor, __data[currentBankNumber], chunkOffset, actualBytesToCopy)
